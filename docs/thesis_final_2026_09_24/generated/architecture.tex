...
\node[civicbox,draw=civicGold,fill=civicGold!8,text width=11.6cm] (rules) at (7,-11.5) {Conditional birth/death rules only:\\score adjustments and correction-fee supplementation\\Passport / BRTA skip these extra rules, not reranking};
...
\end{tikzpicture}
\end{singlespace}
\caption{Current CivicRAG architecture, redrawn from the P2 offline/online organization and updated for three domains. Dashed arrows indicate index access. The gold box distinguishes domain-specific rules from shared reranking. The selected-evidence route bypasses the legacy controlled-answer branch.}\label{fig:architecture}
\end{figure}
