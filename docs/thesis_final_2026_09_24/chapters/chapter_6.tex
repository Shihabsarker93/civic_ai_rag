...
The evaluator uses RAGAS 0.2.15 and actual supplied answer contexts, not every retrieved candidate \cite{ragas,ragasdocs}. There are $30\times2\times3=180$ completed metric scores, all valid, with no missing final scores. The appended Sources footer is removed before evaluation; response wording is otherwise preserved \cite{project_ragas}.
...
A separate experiment compares BM25-only, dense-only and weighted Hybrid RRF on the same 30 questions before reranking, registration boosts and evidence selection \cite{project_retrieval}. No answer generation occurs in this experiment. The top-five result union for each question forms a relevance pool; an assistant reviewed 309 question/passage pairs representing 192 unique passages. The labels are 105 relevant, 199 irrelevant and five uncertain.
...
The metrics follow established ranked-retrieval principles \cite{irbook}. Let $z_{qi}$ indicate whether the chunk at rank $i$ is labelled relevant, and let $R_q$ be the relevant IDs in the judged pool. For each eligible question:
\begin{align}
...
\end{align}
MRR@5 averages RR@5 over eligible questions. Recall is undefined for an empty relevant pool. With binary relevance, the discounted measure is
\begin{equation}
\mathrm{nDCG@5}_{\mathrm{pool}}(q)=
\frac{\sum_{i=1}^{5}z_{qi}/\log_2(i+1)}
{\sum_{i=1}^{\min(5,|R_q|)}1/\log_2(i+1)}.
\end{equation}
Pooled Recall and nDCG use only relevant passages found in the pooled rankings, not exhaustive corpus labels. Missing retrieved positions contribute zero to Precision's fixed denominator of five. Near-duplicate IDs remain distinct scoring units.
...
\input{generated/retrieval_overall}
...
For each automatic metric, define $\delta_q=s_{\mathrm{Civic},q}-s_{\mathrm{Simple},q}$. All 30 pairs have valid scores. The saved evaluator resamples the 30 paired differences with replacement 2,000 times, using seed 20260924, and reports empirical 95\% intervals \cite{project_ragas}. Table~\ref{tab:ragas-intervals} reproduces those saved intervals rather than recomputing a different protocol.
\input{generated/ragas_intervals}
...
\input{generated/retrieval_passport}
\input{generated/retrieval_birth_death_registration}
\input{generated/retrieval_brta}
