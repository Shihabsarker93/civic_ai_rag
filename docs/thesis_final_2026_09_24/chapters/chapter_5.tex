...
Figure~\ref{fig:architecture} presents this modular design. Its offline panel shows corpus and index preparation; the online panel shows the selected-evidence route used in the main CivicRAG comparison. Legacy controlled-answer branches remain available in the repository, but do not generate answers in this route.

\input{generated/architecture}

The diagram shows that source content remains connected to generation through chunk identifiers. The upper stage constructs indexes; it does not train an LLM. Dense search, BM25, rank fusion, reranking, applicability selection and local generation are shared across the three domains. The gold box identifies additional registration-specific adjustments, separating them from shared reranking.
