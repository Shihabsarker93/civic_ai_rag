...
\section{Preliminary Design and Final Specification}
...
\section{Data Collection and Dataset Development}
\subsection{Source Families and Original Preservation}
...
\subsection{Data Reduction and Inclusion}
...
\subsection{Dense Embedding and Search}
...
\subsection{Reranking and Domain-Specific Logic}
...
\subsection{Applicability Selection and Context Budget}
...
\subsection{Answer Generation and Safety Behaviour}
...
\subsection{Matched Simple RAG Baseline}
The isolated runner \code{scripts/run_simple_matched.py} performs dense-only top-six retrieval. It does not call BM25 fusion, reranking, applicability selection, sibling expansion or controlled answers. It applies the same whole-chunk character budget, selected-route generation prompt, Qwen3 settings and language/truncation handling as the comparison configuration. The \code{selected_evidence=True} generator flag chooses the shared prompt; it does not invoke the selector inside the baseline.
